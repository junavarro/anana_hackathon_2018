...
client = new Paho.MQTT.Client("spectacular-hairdresser.cloudmqtt.com", 443, '', "clientId-anana");
...
function onSuccessSubsCallback(data){
  console.log("Subscribe",data);
};

function onFailureSubsCallback(error){
...
var subscribeOptions = {
    ...
    onSuccess: onSuccessSubsCallback,
    onFailure: onFailureSubsCallback,
    timeout: 10
};
...
function onConnect() {
  ...
  client.subscribe("anana2", subscribeOptions);
...
function onMessageArrived(message) {
  ...
  //var listOfData = String(JSON.parse(String(message.payloadString)).image).split(",");
  var listOfData = message.payloadString;
  self.images = JSON.parse(listOfData).image;
  //console.log("onMessageArrived:",JSON.parse(listOfData).image);
...
setInterval(function(){
  ...
  for (var i = 0; i < MapRows; i++) {
    for (var j = 0; j < MapColumns; j++) {
        self.points.push([hexRadius * j * 1.75, hexRadius * i * 1.5]);
    }//for j
  }//for 
